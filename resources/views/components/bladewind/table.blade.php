        ...
        @if($searchable)
            <div class="bw-table-filter-bar bg-slate-100 p-2">
                <x-bladewind::input name="bw-search-{{$name}}" placeholder="{{$search_placeholder}}"
                                    ...
                                    prefix_is_icon="false" add_clearing="false" class="!mb-0 focus:!border-slate-300"
                                    />
            </div>
        @endif
        <table class="bw-table w-full {{$name}} @if($has_shadow) shadow-2xl shadow-gray-200 dark:shadow-xl dark:shadow-white-900 @endif
            ...
            @if(empty($data))
                <thead>
                <tr class="bg-gray-200 dark:bg-white-800">{{ $header }}</tr>
                </thead>
                <tbody>{{ $slot }}</tbody>
            @else
                @if($total_records > 0)
                    <thead>
                    <tr class="bg-gray-200 dark:bg-white-800">
